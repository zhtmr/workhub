...
export function base64Decode(encoded: string): string {
  try {
    ...
    throw new Error("Base64 디코딩 실패: 유효하지 않은 Base64 문자열입니다");
  }
}
...
export function urlDecode(encoded: string): string {
  try {
    ...
    throw new Error("URL 디코딩 실패: 유효하지 않은 URL 인코딩입니다");
  }
}
...
export async function generateHash(
  ...
  try {
    ...
    throw new Error(`${algorithm} 해시 생성 실패`);
  }
}

// MD5 구현 (Web Crypto API는 MD5 미지원, 직접 구현)
