...
export async function startProxyServer(preferredPort: number = 3001): Promise<number> {
  ...
  // ============================================================
  // External API Proxies (Docker, Prometheus, GitLab)
  // Mixed Content 문제를 우회하기 위한 프록시
  // ============================================================

  // Docker API Proxy
  app.post('/api/docker/proxy', async (req: Request, res: Response) => {
    const { host, path, method = 'GET' } = req.body;

    if (!host || !path) {
      res.status(400).json({ success: false, error: 'host and path are required' });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        ...
        res.status(response.status).json({ success: false, error: errorText || `Docker API error: ${response.status}` });
        return;
      }
  ...
  app.post('/api/prometheus/proxy', async (req: Request, res: Response) => {
    const { endpoint, path, params, authToken } = req.body;

    if (!endpoint || !path) {
      res.status(400).json({ success: false, error: 'endpoint and path are required' });
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        const errorText = await response.text();
        res.status(response.status).json({ success: false, error: errorText || `Prometheus API error: ${response.status}` });
        return;
      }
  ...
  // GitLab API Proxy
  app.post('/api/gitlab/proxy', async (req: Request, res: Response) => {
    const { baseUrl, endpoint, method = 'GET', privateToken, body } = req.body;

    if (!baseUrl || !endpoint || !privateToken) {
      res.status(400).json({ success: false, error: 'baseUrl, endpoint, and privateToken are required' });
      return;
    }
